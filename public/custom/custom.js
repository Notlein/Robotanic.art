...
///settings
var gravity = 0.001;  // (rate of y-velocity increase per frame per point mass of 1)
var rigidity = 20;  // global span rigidity (as iterations of position accuracy refinement)
...
///settings
var worldSpeed = 2;//3;  // (as frames per iteration; higher is slower) (does not affect physics iterations)
...
var phr = 3;  // photosynthesis rate ( rate plants store energy from sunlight )
...
function Plant( xLocation ) {
  ...
  this.maxTotalSegments = Tl.rib(10,30);  // maximum total number of segments
...
function Segment( plant, parentSegment, basePoint1, basePoint2 ) {
  ...
  //skins
  this.skins = [];
  this.skins.push( addSk( [ this.ptE1.id, this.ptE2.id, this.ptB2.id, this.ptB1.id ], "transparent" ) );
}
...
function growLeaves( plant, segment ) {
  ...
  if ( s.spLf1.l < p.maxLeaflength ) {
    ...
    if ( s.spF.l > p.maxSegmentWidth*0.6 && !s.hasLeafScaffolding ) {
      ...
      s.spLf1ScC.l += p.leafGrowthRate * 0.36;
      s.spLf1ScD.l += p.leafGrowthRate * 0.36;
      //(leaf 2)
      s.spLf2ScA.l += p.leafGrowthRate * 1.25;
      s.spLf2ScB.l += p.leafGrowthRate * 1.5;
      s.spLf2ScC.l += p.leafGrowthRate * 0.36;
      s.spLf2ScD.l += p.leafGrowthRate * 0.36;
    }
  }
}
